= not7CD =

Student. 

W HS'ie utrzymuję [[projekty:wiki|wiki.hs3.pl]] oraz [[projekty:whois|at.hs3.pl]].

Staram się coś robić z [[wydarzenia:radio:start|radio]], ale życie. 
Szukam pracy w przemyśle kosmicznym. Mogę też programować pralki.

<code>
ssh-rsa AAAAB3NzaC1yc2EAAAADAQABAAACAQDjIi2qvirrTFSrXHrdHlbAqX7wCE2+0iXWFhs7jClw6RPgTbdLjTKjHGFYoA5OTBIDTSXHfRwmM0oLg7nYwY+VeJvHPlJwxx8INBzUYOL3Q0PgBZ1kcSMoZIQZubzBOvb7FMoRCggpzeQd2XGlp/sF2FkF3K3Vw9Zx5CTmtUoyWxpX5lOUg/ujaY4JP4HXTb4ehzItIv'' "not7cd"
</code>
<code>
adduser --shell /bin/zsh --gecos 'Norbert' not7cd
</code>